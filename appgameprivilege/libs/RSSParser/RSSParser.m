...
+ (NSSet *)defaultAcceptableContentTypes {
    ...
    return [NSSet setWithObjects:@"application/xml", @"text/xml",@"application/rss+xml",@"application/atom+xml" , @"text/html" , nil];
}
...
- (void)parser:(NSXMLParser *)parser didEndElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if (currentItem != nil && tmpString != nil) {
        ...
        if ([elementName isEqualToString:@"im:name"]) {
            [currentItem setName:tmpString];
        }
        
        if ([elementName isEqualToString:@"im:image"]) {
            [currentItem setImage:[NSURL URLWithString:[tmpString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
