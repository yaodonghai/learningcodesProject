...
@synthesize willNavigateToIndexBlock;
@synthesize didNavigateToIndexBlock;
@synthesize coachMarksViewWillCleanupBlock;
@synthesize coachMarksViewDidCleanupBlock;
#pragma mark - Methods
...
- (void)setup {
    ...
    self.lblCaption.font = [UIFont fontWithName:@"RTWSYueRoudGoDemo-Regular" size:18.0],
...
- (void)goToCoachMarkIndexed:(NSInteger)index {
    ...
    willNavigateToIndexBlock (self,markIndex);
...
//    }
    ...
    if (self.enableContinueLabel) {
        if (markIndex == 0) {
            lblContinue = [[UILabel alloc] initWithFrame:(CGRect){{0, self.bounds.size.height - 60.0f}, {self.bounds.size.width, 60.0f}}];
            lblContinue.font = [UIFont boldSystemFontOfSize:17.0f];
            lblContinue.textColor = [UIColor orangeColor];
...
- (void)cleanup {
    coachMarksViewWillCleanupBlock (self);
...
//    }
    ...
    [UIView animateWithDuration:self.animationDuration
                     ...
                     completion:^(BOOL finished) {
                         ...
                         coachMarksViewDidCleanupBlock (self);
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
    didNavigateToIndexBlock (self,markIndex);
